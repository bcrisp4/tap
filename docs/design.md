...
| Frontend framework | Svelte 5 + SvelteKit (`@sveltejs/adapter-static`) | Small bundles, runes-based reactivity, fully prerendered SPA embedded into the Go binary |
| Frontend data layer | `@tanstack/svelte-query` + `@tanstack/query-async-storage-persister` + `idb` | Cache, optimistic mutations with rollback, persisted offline mutation queue replayed on reconnect — backed by IndexedDB |
| Database | SQLite (WAL mode) via `modernc.org/sqlite` | Pure-Go driver — no CGo, smaller image, trivial cross-compile, full FTS5 support |
| Full-text search | FTS5 (SQLite extension) | BM25 ranking, prefix queries, contentless mode |
| Content extraction | `go-readability` (Readeck fork, Apache-2.0) | Maintained Mozilla Readability port; consumed as a Go module |
| Feed parsing | `mmcdole/gofeed` | RSS 1.0/2.0, Atom, JSON Feed |
| HTML sanitizer | `microcosm-cc/bluemonday` + Tap pre/post pass | Battle-tested allowlist core; Tap layer adds proxy URL rewriting, iframe host policy, 1×1 strip |
| Container base | `gcr.io/distroless/static-debian12` (or `scratch`) | Pure-Go binary; no libc needed |

### Pure Go build

Tap builds with `CGO_ENABLED=0`. SQLite runs through `modernc.org/sqlite`
(a pure-Go port with full FTS5 support), and no other dependency requires
C. The binary is fully static, ships in a `distroless/static` or `scratch`
image, and cross-compiles to any Go target without a toolchain swap.
...
- `user_id` — denormalised from feeds for query efficiency on the unread view
...
| Query                         | Index                              |
| ----------------------------- | ---------------------------------- |
| Unread (chronological)        | `idx_entries_user_unread`          |
...
2. **Extract** with three-tier precedence:
   ...
   2. *(v2)* Built-in rules table for known domains — extension point,
      not shipped in v1.
   3. Fallback: `go-readability` (Readeck fork, Apache-2.0) — a maintained
      Mozilla Readability port consumed as a Go module.
...
**Sanitization policy.** The allowlist is enforced by
`microcosm-cc/bluemonday` (battle-tested, OWASP-aligned). Tap wraps it in
a small pre-pass for the Tap-specific concerns bluemonday doesn't
natively express (proxy URL rewriting, relative-URL resolution, iframe
`src` host check, 1×1 image strip) and configures bluemonday's policy to
...
- List endpoints: `{ "data": [...], "pagination": { "limit": N, "offset": N, "total": N } }`
- Single-resource endpoints: the bare object
- Errors: `{ "error": { "code": "...", "message": "..." } }` with the
  appropriate HTTP status
- All timestamps are unix epoch seconds (matching the schema)
...
Stage 2: Go             → copy assets into embed dir, CGO_ENABLED=0 go build
Stage 3: distroless/static → copy binary; no shell, no libc

Target: < 25MB final image (pure Go, fully static)
```

The binary is statically linked and runs unmodified on any Linux
platform Go targets.
...
1. **Command-line flags** — e.g. `--db-path /var/lib/tap/tap.db`
2. **Environment variables** — e.g. `TAP_DB_PATH=/var/lib/tap/tap.db`
3. **Config file** — YAML format. Default search path:
   `/etc/tap/config.yaml` then `./tap.yaml`. Override via
...
`ff.Command` provides the subcommand tree (`tap serve`, `tap migrate`,
`tap import-opml`, …).

| Variable | Default | Description |
|----------|---------|-------------|
| `TAP_DB_PATH` | `./tap.db` | SQLite database file path |
| `TAP_LISTEN` | `127.0.0.1:8080` | HTTP listen address. Local-only by default — v1 has no built-in auth, so expose via reverse proxy or override to `:8080` / `0.0.0.0:8080` only on trusted networks |
...
- **Miniflux** — architecture, adaptive polling, sanitization rules,
  tombstone pattern, privacy approach. Tap uses `go-readability` (Readeck
  fork) for content extraction rather than vendoring Miniflux's port.
