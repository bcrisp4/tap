...
- **EntryRow ownership.** `EntryRow.svelte` is M1's primitive — used by Unread (M2) and History (M8). M3 does **not** consume `EntryRow`; the Saved row is a view-specific component (`SavedRow.svelte`, owned by M3) because the design's `.ts-saved-row` chrome (bookmark rail-mark, hover-reveal action strip, "published <date>" eyebrow, dedicated `is-read` styling) diverges from the generic `.entry` row enough that a shared primitive would bloat its API for one consumer. Decision recorded 2026-05-11 by team-lead; umbrella spec §3.2 is being amended to match.
...
1. **EntryRow / SavedRow ownership — resolved 2026-05-11 by team-lead.** Decision: `EntryRow.svelte` stays in M1's primitive library, used by Unread (M2) and History (M8). `SavedRow.svelte` is M3-owned and view-specific (not a shared primitive) because `.ts-saved-row` chrome diverges enough from `.entry` that folding it into a variant prop would bloat the primitive's API for one consumer. Umbrella spec §3.2 is being amended to reflect this; the team-lead decision is authoritative regardless of spec edit timing. No further coordination needed.
...
- **§3.2 primitive consumption ("every view uses these")** — `Saved.svelte` consumes `EmptyState`, `KbdChip`, `FeedAvatar` from M1 directly. `SavedToolbar.svelte` consumes `KbdChip`. Per team-lead's 2026-05-11 decision, `EntryRow` is NOT a Saved-view primitive; `SavedRow.svelte` + `SavedMobileRow.svelte` + `SavedToolbar.svelte` are M3-owned view-specific components, not entries in M1's primitive library. The umbrella spec §3.2 amendment in flight records this.
