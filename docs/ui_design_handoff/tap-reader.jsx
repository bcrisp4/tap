    ...
    <div className="reader-header">
      ...
      <div className="reader-actions">
        ...
        <button className="reader-action is-saved" title="Saved (s)">
          <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" strokeLinecap="round"><path d="M4 2.5h8v11l-4-3-4 3z"/></svg>
          <span>Saved</span>
        </button>
...
function DesktopReader({ entries, feeds, entryId = 101, theme = "light", fontMode = "serif", measure = "comfortable" }) {
  ...
  return (
    <div className={`tap theme-${theme} measure-${measure}`} style={{ display: 'flex', height: '100%', ...fontStyle }}>
      <Sidebar active="unread" feeds={feeds} unreadCounts={Object.fromEntries(feeds.map(f => [f.id, entries.filter(e => e.feed===f.id && !e.read).length]))} />
      {/* Mini list rail */}
      <aside className="reader-rail">
        <div className="rail-head">
          <span className="mono">UNREAD · 11</span>
        </div>
        {entries.slice(0, 8).map((e) => {
          const f = feeds.find((x) => x.id === e.feed);
          const sel = e.id === entryId;
          return (
            <div key={e.id} className={`rail-row ${sel ? 'is-selected' : ''} ${e.read ? 'is-read' : ''}`}>
              <span className="rail-dot" aria-hidden="true"></span>
              <div className="rail-text">
                <div className="rail-title">{e.title}</div>
                <div className="rail-meta sans">{f.name} · {e.ago}</div>
              </div>
            </div>
          );
        })}
