...
| `web/src/styles/global.css` | `@font-face` declarations (Source Serif 4, Inter Tight, JetBrains Mono — vendored locally), body reset, `:focus-visible` outline rule, scrollbar, `prefers-reduced-motion` guard, `@keyframes tap-pulse` and `tf-spin` |
...
6. **Font hosting.** The design uses Google Fonts in the mockups. The Tap binary is meant to work offline / on a self-hosted server with no third-party fetches. Foundations milestone vendors the three font families (Source Serif 4, Inter Tight, JetBrains Mono) into `web/src/assets/fonts/` and serves them from the same origin. `@font-face` declarations live in `global.css`.
