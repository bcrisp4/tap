...
func TestRegisterFlags_Defaults(t *testing.T) {
	...
	require.Empty(t, cfg.IframeAllowlist)
}
...
func TestRegisterFlags_FileBeatsDefault(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(path, []byte("db-path: /file/x.db\nlisten: 0.0.0.0:9000\n"), 0o644))
...
func TestRegisterFlags_YAMLListPopulatesStringList(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "tap.yaml")
	require.NoError(t, os.WriteFile(path, []byte("allowed-hosts:\n  - a.example.com\n  - b.example.com\n"), 0o644))

	cfg := parse(t, nil, path)
	require.Equal(t, []string{"a.example.com", "b.example.com"}, cfg.AllowedHosts)
}
